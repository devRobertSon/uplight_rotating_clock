...
| D28 | 하부 보스 방향 | 하부 캡의 보스를 **드럼 내부 방향(위)** 으로 배치, 드럼 가시 영역(R<10 중심부)과 높이 공유. 하부 캡은 영구 고정 (M3은 초기 조립 시 한 번만 잠금) | 보스 24 mm 외부 추가 → 12 mm 절감. envelope 147 → 127 mm. Frame 측면 기둥 160 → 140 mm 회복. 상부 캡은 보스 외부 유지 → M3 접근·패널 교체 (D13 탈착 반복) 보장. CAD 모델 변경 없음 (조립 방향만 변경 — 하부 캡을 뒤집지 않음) |
