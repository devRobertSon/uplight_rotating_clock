...
| `slotCenterX_90` | Length | `0 mm` | 슬롯 0의 중심 X (slot 0은 -Y 축에 위치하므로 X=0) |
| `slotCenterX_60` | Length | `0 mm` | 동일 |
| `slotCenterY_90` | Length | `-(#apothem_90 - #slotWidth / 2)` | 슬롯 0의 중심 Y = -41.20 mm |
| `slotCenterY_60` | Length | `-(#apothem_60 - #slotWidth / 2)` | 슬롯 0의 중심 Y = -25.42 mm |
...
| 슬롯 | 3.2(radial) × 27.8(tangential) × 10개 | 폴리곤 변에 평행, 변에서 안쪽으로 3.2 깊이. **상부 캡 깊이 3 mm (블라인드)**, 하부 캡 6 mm 관통 |
...
| 직사각형 (Slot/Rectangle) | 폭(radial) / 길이(tangential) | 그대로 | `#slotWidth` × `#panelWidth_90` |
...
- 중심점 좌표 칸에 **변수로 입력**: X = `#slotCenterX_90`, Y = `#slotCenterY_90`
  - 자동 계산: (0, -41.20)
  - 변수 안 쓰고 직접 입력해도 동일하지만, ∅60 캡 재사용·치수 변경 추적을 위해 변수 권장
- 직사각형 두 치수 명시:
  - **Width (X 방향, tangential)** = `#panelWidth_90` (≈ 27.81 mm) ← **긴 변**
  - **Height (Y 방향, radial)** = `#slotWidth` (= 3.2 mm) ← **짧은 변**
- 결과: 슬롯이 캡의 **-Y 변에 평행하게 누운** 가로로 긴 1개. 외측 긴 변이 폴리곤 변에 거의 닿음.
- ✅ 검증: 슬롯이 "폴리곤 한 변과 평행한 작은 띠" 모양인지 눈으로 확인. "캡 중심을 향해 깊숙이 박힌 막대"면 잘못 (이건 패널 *높이* 방향과 혼동한 것).

> **Tip 1**: 슬롯 외측 긴 변을 폴리곤 -Y 변에 **Coincident** constraint 잡으면 Y=-41.2 자동 결정됨.
> **Tip 2**: 슬롯 가로 길이는 폴리곤 변 길이와 같으므로 두 끝점에 Coincident도 가능 (단, 변수 사용보다 덜 유연).
...
| 슬롯이 캡 중심을 향해 깊숙이 박힌 긴 막대로 그려져 회전 패턴 시 겹침 | 패널 *높이*(55)와 *폭*(27.8)을 혼동. top view 슬롯은 패널 단면 (3 × 27.8) 그대로 — **폭 3.2 (radial) × 길이 27.8 (tangential, 폴리곤 변과 평행)**. 위치 `(#slotCenterX_90, #slotCenterY_90)` |
| 슬롯 외측 긴 변이 폴리곤 변에서 떨어져 있음 | 위치 Y가 정확히 `#slotCenterY_90` (= -41.20) 가 아님. Coincident constraint로 폴리곤 변에 붙이면 자동 |
