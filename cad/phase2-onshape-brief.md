...
> **핵심**: 샤프트는 본체 상부 (+Y쪽)에 위치, 본체 중심은 샤프트에서 -Y로 ~8mm 어긋남. 마운트 홀(∅4, 35mm 간격)은 **본체 중심 라인**에 위치 = 샤프트 기준 Y = -8.
...
| 평면 | Top |
| 도구 | Center point rectangle |
| 중심 | 원점 (= 샤프트 위치, Coincident) |
| Width (X) | **50 mm** (마운트 홀 정렬축) |
| Depth (Y) | **40 mm** (본체 + 커넥터 클리어런스 수용) |

→ Sketch 종료. 플레이트는 Y 방향으로 -20~+20, 본체 (-22~+6)는 -2mm만 살짝 벗어나 커넥터·케이블 출구 확보.
...
플레이트 윗면(z=3) 클릭 → New Sketch. 원 3개 (위치 도면 기반):

| 홀 | 위치 (X, Y) | Diameter |
|---|---|---|
| 모터 샤프트 통과 | **(0, 0)** = 원점 | **6 mm** (∅5 + 1 clearance) |
| 모터 마운트 좌 | **(-17.5, -8)** | **3.2 mm** (M3 clearance) |
| 모터 마운트 우 | **(+17.5, -8)** | 3.2 mm |

> ⚠ 이전 버전에서 마운트 홀을 (±17.5, 0)에 두었으나 **도면 재확인 결과 Y = -8** (본체 중심 라인). 정정.
...
- 모터 샤프트가 본체 중심에서 +Y로 8mm 어긋남 → 플레이트 중심(샤프트 = 원점) 기준으로 모터 본체는 -Y쪽으로 치우쳐 매달림. **§8 Frame Section** 설계 시 본체·커넥터가 플레이트 아래(-Y)에 위치함을 반영.
...
- 측면 기둥 × 2: 폭 5 × 깊이 10, **높이 140** (D28 — 하부 보스를 드럼 내부 방향으로 배치하여 드럼과 높이 공유, envelope 127mm + 13mm 버퍼)
