...
| 허브 | 중앙 ∅20 영역 (별개 보스 아님, 단순 표식) | 별도 피처 불필요 |
| 슬롯 | 3.2×28 관통 × 10개 | R = 17 ~ R = 45 (28mm 길이 × 36° 피치) |
...
| 정다각형 (Polygon) | 꼭짓점 외접원 지름 또는 변 길이 | 지름값 그대로 | `#drumDiameter_90` |
| 직사각형 (Slot/Rectangle) | 폭 / 길이 | 그대로 | `#slotWidth`, `28 mm` |
...
1. **외형: 정십각형**
   ...
   - 옵션: **Inscribed in circle** 또는 **Circumscribed about circle** 중 **Circumscribed (꼭짓점 외접)** 선택
   - 중심: 원점 (0, 0)에 클릭
...
- 디스크 윗면 클릭 → New Sketch
- 슬롯 0 위치(전면 = X축 음수 방향, 즉 도면상 "아래")에 직사각형 1개:
  - Toolbar → `Center point rectangle`
  - 중심점: X = 0, Y = `-(#drumDiameter_90 / 2 - 14 mm)` 정도... 사실 중심점은 슬롯 길이의 가운데이므로 `Y = -31` (= -(45+17)/2)
  - 더 명확하게: 슬롯 외측 끝이 R=45 - 0.2(외측 lip 1mm 미만), 내측 끝이 R=17. **단순화: 외측 끝 = 폴리곤 내경에 살짝 못 미치게, 내측 끝 = R=17**.
  - 실용: 중심점 좌표 `(0, -(slotOuterR + slotInnerR)/2)` 인데 slotOuterR을 변수로 만들 수도. **간이 권장**: 중심점을 `(0, -31 mm)` 로 직접 입력 후, 폭 = `#slotWidth` (= 3.2), 길이(=Y 방향 치수) = `28 mm`
- 두 차원 치수 명시:
  - **Width (X 방향)** = `#slotWidth`
  - **Length (Y 방향)** = `28 mm` (또는 변수 `slotLength` 신규 추가 가능)

> **Tip**: 슬롯 외측 끝을 폴리곤 변에 맞추려면 Coincident constraint 사용 (한 변이 폴리곤 변에 닿도록). 그러면 길이 28mm는 자동으로 나옴 (폴리곤 변 → 안쪽 17mm 지점).

#### Step 5 — Extrude Cut "slot 0" (Through All)
- Step 4 스케치 선택 → 직사각형 영역
- 작업: **Remove** (= Cut)
- 끝 조건: **Through all**
- → 결과: 슬롯 0 자리에 3.2×28 관통 구멍

#### Step 6 — Circular Pattern "10 slots"
- Toolbar → `Circular Pattern` (피처 패턴, 스케치 패턴 아님)
- 패턴 대상: Step 5의 Extrude Cut 피처 선택
- 회전 축: 디스크 중심축 (Y축 또는 베이스 스케치의 중심 원/축 선택)
- **Instance count** = `#drumFaces_90` (= 10)
- **Equal spacing**: `360 deg` 등간격 (즉 36° 피치 자동)
- → 결과: 10개 슬롯이 36° 간격으로 모두 관통

#### Step 7 — Sketch "M3 insert hole" (Right plane, 또는 Top plane에 평행한 측면 평면)
M3 세트스크류 홀: **외주 → 허브 → 축 수평 관통**, 캡 두께 중심선(z=3mm)에 위치.

작업 평면 선택:
- Right plane (= YZ 평면, 캡을 옆에서 본 단면)
- 또는 새 Plane 만들기: Top plane을 Z축 방향으로 `#capThickness / 2` (= 3mm) 오프셋 → "Mid Plane"

권장: **Right plane**(원점·Y축·Z축이 정의된 평면)에 스케치.

스케치 내용:
- Circle 도구로 원 1개:
  - 중심: 캡 외주(폴리곤 한 변의 외측) 바로 너머 + 두께 중심선 → 좌표 `(X = #drumDiameter_90 / 2, Z = #capThickness / 2)` = (45, 3)
  - 정확히는 폴리곤 외접원이 ∅90이므로 한 변 중점은 외접원 안쪽으로 약간 들어옴. 단순화: X = 45 (외주 바로 바깥), Z = 3
  - 실은 스케치 평면이 측면이라 Y가 깊이축. 좌표 표시는 (Y, Z): 중심 = `(0, 3)` (Y=0은 X축선, Z=3은 두께 중간)
  - **간단히**: 디스크 옆에서 본 측면 스케치 평면에서, 디스크 중심선과 평행한 새 직사각형 슬라이스 위치
- Diameter 치수: `#insertHole` (= 4.2)

> **이 단계는 시각화 어려우니 다른 방법 권장 — Hole 피처 사용**:
> 1. 캡 외주의 한 다각형 변 면(rib face) 클릭 → New Sketch
> 2. 그 면 위에 점 1개: Z = `#capThickness / 2` (두께 중심), X = 변의 중앙
> 3. Sketch 종료
> 4. Toolbar → `Hole` 피처
> 5. Hole point: 위 점 선택
> 6. Type: **Simple**, Diameter: `#insertHole`
> 7. End condition: **Blind in last**, Depth: `#drumDiameter_90 / 2 - #shaftHole / 2` (= 42.4 mm) 또는 **Up to next** (가장 가까운 다음 면 = 축 홀에서 멈춤)
