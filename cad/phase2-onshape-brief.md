...
> ⚠️ **방향 규칙**: 슬롯의 **긴 변(28mm)은 반드시 원점 → 슬롯 중심 방향(radial)과 평행**해야 함.
> 원주 둘레 방향(tangent)으로 누이면 회전 패턴 시 인접 슬롯과 겹침.
>
> ```
> 잘못 (tangent, 막대가 누움)        올바름 (radial, 막대가 찌름)
>
>     ┌─┐                               (텅 빔)
>     │ │
>     │ │   ← Y(28) tangent              ●─────→ +X
...
- 중심점 좌표를 **반드시 X=0**, **Y=-31 mm** 로 입력 (캡 정면 = -Y 방향 기준)
- 직사각형이 그려지면:
  - **Width (X 방향)** = `#slotWidth` (= 3.2 mm) ← 짧은 변, tangent
  - **Length (Y 방향)** = `28 mm` ← 긴 변, radial
- 결과: 슬롯이 캡의 **-Y 방향(아래쪽)** 에 세로로 길게 1개. R = 17 ~ R = 45 범위 점유
- ✅ 검증: 슬롯이 "캡 중심을 향해 찌르는 화살표" 모양인지 눈으로 확인. "둘레를 따라 누운 막대"면 잘못.

> **Tip**: 직접 좌표 입력이 어렵다면 슬롯 외측 끝을 폴리곤의 -Y 변에 Coincident, 슬롯 내측 끝을 R=17 보조원에 Coincident 로 잡으면 길이 28mm가 자동 결정됨.
...
| 슬롯이 회전 패턴 후 인접 슬롯과 부채꼴이 아닌 **꼬임/겹침**으로 나옴 (슬롯 0이 캡 측면에 세로 막대로 누워있을 때) | 슬롯의 긴 변(28)이 tangent로 누워있음. 긴 변이 radial(원점→슬롯중심 방향)과 평행이 되도록 90° 회전하거나, 슬롯 0을 (0, -31)에 다시 그려 -Y 방향으로 세움 |
