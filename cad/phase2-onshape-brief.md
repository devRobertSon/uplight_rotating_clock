...
| 캡 variant | 끝 조건 | 깊이 |
|---|---|---|
| 상부 (`Top Cap` config) | **Blind** | `#slotDepth_top` = 3 mm (관통 X) |
| 하부 (`Bottom Cap` config) | **Through all** 또는 Blind | `#slotDepth_bottom` = 6 mm (관통 O) |
...
#### Step 8 — Configuration 만들기 (상부/하부)

> ⚠ Onshape Configuration **Variable** 타입에는 **Boolean이 없음** (Length·Angle·Integer·Real·Text 5종). 별도 Variable을 만들지 말고, **Configuration 항목 자체**를 둘 추가한다.

1. 우측 `Configurations` 패널 헤더 우측의 **큰 `+`** 클릭 (= "Add configuration")
   - 작은 "+ variable" 류 버튼은 누르지 말 것 (Length/Angle 등 Variable 추가용)
2. 첫 Configuration 이름을 **`Top Cap`** 으로 입력
3. 다시 `+` → 두 번째 Configuration 이름 **`Bottom Cap`**
4. 패널 상단에 두 토글 (`Top Cap` | `Bottom Cap`) 표시 확인

```
Configurations 패널
├── Top Cap       (선택 시 자석 포켓 없는 상부 캡)
└── Bottom Cap    (선택 시 자석 포켓 있는 하부 캡)
```

이 설정만으로는 아직 두 모델이 동일. Step 11에서 자석 포켓 피처를 `Bottom Cap` 에서만 활성으로 묶어 차이를 만든다.
...
#### Step 11 — Configuration: 자석 포켓 피처를 `Bottom Cap` 에서만 활성
- 피처 트리에서 Step 9 스케치 우클릭 → `Configure feature` → Suppression 설정
- `Top Cap` 선택 시 → **Suppressed**
- `Bottom Cap` 선택 시 → **Active**
