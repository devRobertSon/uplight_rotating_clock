...
| `slotCenterY_90` | Length | `-(#apothem_90 - #slotWidth / 2)` | 슬롯 중심 Y = -41.20 mm |
| `slotCenterY_60` | Length | `-(#apothem_60 - #slotWidth / 2)` | 슬롯 중심 Y = -25.42 mm |
...
- 중심점 좌표를 **X=0**, **Y=-41.2 mm** 로 입력
  - 또는 변수로 `(0, #slotCenterY_90)` (변수 추가했다면)
- 직사각형 두 치수 명시:
...
| 슬롯이 캡 중심을 향해 깊숙이 박힌 긴 막대로 그려져 회전 패턴 시 겹침 | 패널 *높이*(55)와 *폭*(27.8)을 혼동. top view 슬롯은 패널 단면 (3 × 27.8) 그대로 — **폭 3.2 (radial) × 길이 27.8 (tangential, 폴리곤 변과 평행)**. 위치 (0, -41.2) |
| 슬롯 외측 긴 변이 폴리곤 변에서 떨어져 있음 | 위치 Y가 정확히 `-(apothem - slotWidth/2)` = -41.2 가 아님. Coincident constraint로 폴리곤 변에 붙이면 자동 |
